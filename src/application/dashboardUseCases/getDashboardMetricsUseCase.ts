...
export class GetDashboardMetricsUseCase {
  ...
  async execute(
    ...
    try {
      const [
        ...
        this.detectCurrency(orgId),
        this.getInventorySummary(orgId),
        this.getLowStockCount(orgId),
        this.getMonthlySales(orgId, monthStart),
        this.getSalesTrend(orgId, sevenDaysAgo),
        this.getTopProducts(orgId, monthStart),
        this.getStockByWarehouse(orgId),
  ...
  private async detectCurrency(orgId: string): Promise<string> {
    const product = await this.prisma.product.findFirst({
      where: { orgId, currency: { not: null } },
      select: { currency: true },
    });
    return product?.currency || 'COP';
  }

  private async getInventorySummary(orgId: string) {
  ...
  private async getMonthlySales(orgId: string, monthStart: Date) {
  ...
  private async getSalesTrend(orgId: string, sevenDaysAgo: Date) {
    const result = await this.prisma.$queryRaw<
      Array<{ date: Date; count: bigint; revenue: string }>
    >`
      SELECT
  ...
  private async getTopProducts(orgId: string, monthStart: Date) {
    const result = await this.prisma.$queryRaw<
  ...
  private async getStockByWarehouse(orgId: string) {
    const result = await this.prisma.$queryRaw<
