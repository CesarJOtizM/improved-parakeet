...
import { SALE_NOT_FOUND, MOVEMENT_NOT_FOUND } from '@shared/constants/error-codes';
import {
  BusinessRuleError,
  DomainError,
  err,
...
export class GetSaleMovementUseCase {
  ...
  async execute(
    ...
    if (!movement) {
      return err(
        new NotFoundError(`Movement with ID ${sale.movementId} not found`, MOVEMENT_NOT_FOUND)
      );
    }
