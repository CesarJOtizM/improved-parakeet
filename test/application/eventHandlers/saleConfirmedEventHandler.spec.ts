...
describe('SaleConfirmedEventHandler', () => {
  let handler: SaleConfirmedEventHandler;
  ...
  beforeEach(() => {
    ...
    handler = new SaleConfirmedEventHandler(mockAuditRepository);
  ...
  describe('handle', () => {
    it('Given: SaleConfirmed event When: handling event Then: should log audit information', async () => {
      // Arrange
      ...
      // Assert
      expect(loggerSpy).toHaveBeenCalledWith('Handling SaleConfirmed event', {
        saleId: 'sale-123',
        saleNumber: 'SALE-001',
        movementId: 'mov-456',
        orgId: 'org-123',
      ...
      expect(loggerSpy).toHaveBeenCalledWith('Sale confirmation audit logged successfully', {
        saleId: 'sale-123',
        saleNumber: 'SALE-001',
        movementId: 'mov-456',
      });
    ...
    it('Given: SaleConfirmed event without movementId When: handling event Then: should log success message', async () => {
      // Arrange
      const mockSale = createMockSale({ id: 'sale-456' });
      mockSale.saleNumber.getValue = () => 'SALE-002';
      const event = new SaleConfirmedEvent(mockSale);
