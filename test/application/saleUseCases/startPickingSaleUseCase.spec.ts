...
describe('StartPickingSaleUseCase', () => {
  const mockOrgId = 'test-org-id';
  const mockSaleId = 'sale-123';
  const mockUserId = 'user-456';

  let useCase: StartPickingSaleUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new StartPickingSaleUseCase(
      mockSaleRepository,
      mockOrganizationRepository,
      mockEventDispatcher
    );
  ...
  const createMockSale = (status: string = 'CONFIRMED') => {
  ...
  describe('execute', () => {
    it('Given: confirmed sale with picking enabled When: starting picking Then: should return success result', async () => {
      // Arrange
      const mockOrg = createMockOrganization(true);
      const mockSale = createMockSale('CONFIRMED');
      mockSale.startPicking = jest.fn();
      ...
      result.match(
        value => {
          ...
          expect(value.message).toBe('Sale picking started successfully');
        },
        () => {
    ...
    it('Given: non-existent organization When: starting picking Then: should return NotFoundError', async () => {
      // Arrange
    ...
    it('Given: organization with picking disabled When: starting picking Then: should return BusinessRuleError', async () => {
      // Arrange
    ...
    it('Given: non-existent sale When: starting picking Then: should return NotFoundError', async () => {
      // Arrange
      const mockOrg = createMockOrganization(true);
      mockOrganizationRepository.findById.mockResolvedValue(mockOrg);
    ...
    it('Given: sale not in CONFIRMED status When: starting picking Then: should return BusinessRuleError', async () => {
      // Arrange
      const mockOrg = createMockOrganization(true);
      const mockSale = createMockSale('CONFIRMED');
      mockSale.startPicking = jest.fn(() => {
      ...
      result.match(
        ...
        error => {
          expect(error).toBeInstanceOf(BusinessRuleError);
          expect(error.message).toContain('cannot start picking');
        }
      );
    });

    it('Given: draft sale When: starting picking Then: should return BusinessRuleError', async () => {
      // Arrange
      const mockOrg = createMockOrganization(true);
    ...
    it('Given: confirmed sale When: starting picking Then: should call save on repository', async () => {
      // Arrange
      const mockOrg = createMockOrganization(true);
      const mockSale = createMockSale('CONFIRMED');
      mockSale.startPicking = jest.fn();
    ...
    it('Given: confirmed sale When: starting picking Then: should dispatch domain events', async () => {
      // Arrange
      const mockOrg = createMockOrganization(true);
      const mockSale = createMockSale('CONFIRMED');
      mockSale.startPicking = jest.fn();
    ...
    it('Given: confirmed sale When: starting picking with userId Then: should pass userId to startPicking method', async () => {
      // Arrange
      const mockOrg = createMockOrganization(true);
      const mockSale = createMockSale('CONFIRMED');
      mockSale.startPicking = jest.fn();
      ...
      // Assert
      expect(mockSale.startPicking).toHaveBeenCalledWith(mockUserId);
    });

    it('Given: confirmed sale When: starting picking successfully Then: response should contain correct sale data', async () => {
      // Arrange
      const mockOrg = createMockOrganization(true);
      const mockSale = createMockSale('CONFIRMED');
      mockSale.startPicking = jest.fn();
